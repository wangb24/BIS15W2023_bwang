...
```{r}
mammals_data <- mammals %>%
    filter(genus %in% c("Lophocebus", "Erythrocebus", "Macaca")) %>%
    select(mass, gestation, newborn, genus)

head(mammals_data, n=10)
summary(mammals_data)
```

```{r}
minimum_newborn <- min(mammals_data$newborn)
filter(mammals_data, newborn <= minimum_newborn)
```
