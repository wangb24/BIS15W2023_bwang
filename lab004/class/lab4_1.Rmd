...
<!-- ^ created a new dataframe with only variables of interest -->
...
```{r}
anyNA(mammals) %>% print
print(mean(mammals$`max. life`))
```
<!-- In this dataset, NA is shown with "-999" -->
...
```{r}
head(mammals) %>% print
mean(mammals$`max. life`, na.rm=TRUE) %>% print
```
...
```{r}
head(select(mammals, -order, -family), n=3)
```
