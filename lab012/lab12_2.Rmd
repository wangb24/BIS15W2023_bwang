...
ui <- fluidPage(
  selectInput(
    "taxon",
    "Select Taxon",
    choices = c("all", unique(homerange$taxon)),
    selected = "all"
  ),
  plotOutput("plot", width = "75vw", height = "55vh")
)
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    if (input$taxon == "all") {
      ...
      dat <- homerange %>% filter(taxon == input$taxon)
    }

    dat %>%
      ggplot(aes(x = log10.hra)) +
      geom_density(fill = "lightblue", color = "black", alpha = .6) +
      theme_light(base_size = 18) +
      labs(
        title = paste("Density Plot of Log10 Home Range Area by",
          input$taxon, "taxon"),
        x = "Log10 Home Range Area",
        y = "Density"
      )
  })
}
...
ui <- dashboardPage(
  ...
  dashboardBody(
    fluidRow(
      ...
      box(
        title = "Plot of Penguins Data", width = 7,
        plotOutput("plot", width = "600px", height = "500px")
      ) # close the second box
    ) # close the row
  ) # close the dashboard body
