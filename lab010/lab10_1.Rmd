...
life_history %>%
  ...
  ggplot(aes(x = reorder(order, mean_mass), y = mean_mass)) +
  geom_col() +
  scale_y_log10() + # log10 transformation
  coord_flip() +
  labs(title = "Mean Mass by Order",
       x = "Order",
       y = "Mean Mass (g)")
...
elephants %>%
  ...
  theme(plot.title = element_text(size = rel(1.25), face = "bold", hjust = .5))
```
...
life_history %>%
  filter(gestation >= 20) %>%
  select(family, genus, species, gestation, mass) %>%
  arrange(desc(mass))
```
## Practice
