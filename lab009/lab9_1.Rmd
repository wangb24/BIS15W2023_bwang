...
```{r}
homerange <- read_csv(here("lab009", "data", "Tamburelloetal_HomeRangeDatabase.csv"))
```
...
```{r}
ggplot(homerange) + 
  geom_point(mapping = aes(x = log10.mass, y = log10.hra))
```
...
```{r}
ggplot(homerange) +
  geom_jitter(mapping = aes(x = log10.mass, y = log10.hra))
```
...
mammals %>%
  ...
  ggplot(aes(y = fct_reorder(common.name, -log10.mass), x = log10.mass)) +
  geom_col() +
  labs(x = "log10 mass", y = "common name", title = "Top 10 smallest mammals")
```

```{r}
mammals %>%
  slice_min(log10.mass, n = 10) %>%
  ggplot(aes(x = fct_reorder(common.name, -log10.mass), y = log10.mass)) +
  geom_col() +
  labs(x = "common name", y = "log10 mass", title = "Top 10 smallest mammals") +
  coord_flip()
```
