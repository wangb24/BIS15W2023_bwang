...
amniota_clean <- amniota %>%
  clean_names() %>%
  mutate(across(where(is.character) & !common_name, as.factor))
amniota_clean[amniota == -999] <- NA
head(amniota_clean)
```
...
> NA were represented as `-999` in this data. 
