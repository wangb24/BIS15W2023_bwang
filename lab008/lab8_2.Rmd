...
```{r}
expenditures_tidy_l <- expenditures %>%
  rename(category = expenditure) %>%
  pivot_longer(
    cols = -category, 
    names_to = "year",
...
```{r}
expenditures_tidy_l %>%
  filter(category == "Medical and Health") %>%
  arrange(expenditure)
```
...
## `pivot_wider()`
The opposite of `pivot_longer()`. You use `piot_wider()` when you have an observation scattered across multiple rows. In the example below, `cases` and `population` represent variable names not observations.  
