...
msleep %>%
    ...
    summarize(
        total = n(),
        unique_vore = n_distinct(vore)
    ) %>%
    filter(unique_vore > 1)
```
...
msleep %>%
    ...
    summarize(
        mean_bodywt = mean(bodywt), sd_bodywt = sd(bodywt),
...
msleep %>%
    ...
    summarize(
        ...
        mean_brainwt_omit_na = mean(brainwt, na.rm = TRUE), 
    )
```

1. What does `NA` mean? How are NA's being treated by the summarize function?
```{r}
msleep %>%
...
1. Try running the code again, but this time add `na.rm=TRUE`. What is the problem with Cetacea? Compare this to Carnivora. 
```{r}
msleep %>%
