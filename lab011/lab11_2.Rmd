...
```{r message=FALSE, warning=FALSE}
homerange <- read_csv(here("lab011", "data", "Tamburelloetal_HomeRangeDatabase.csv"))
```
...
p + theme_classic() +
  ...
  labs(
    title = "Observations by Taxon in Homerange Data",
    x = NULL,
    y = "n",
    fill = "Trophic Guild"
  )
...
plt <- homerange %>%
  ...
  scale_color_paletteer_d("rcartocolor::Prism") +
...
ecosphere %>%
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
  labs(
    title = "Mass by Diet and Migratory Strategy",
    x = NULL,
    y = "Log 10 Mass",
...
## Practice
1. Build a faceted plot that shows a distribution of ~~life expectancy~~ log10.mass by habitat type.
```{r}
ecosphere %>%
  ggplot(aes(x = log10_mass, color = habitat, fill = habitat)) +
...
```{r}
ecosphere %>%
  ggplot(aes(x = life_expectancy, color = habitat, fill = habitat)) +
  geom_histogram(alpha = 0.3, stat = "count") +
  facet_grid(. ~ habitat)
```
