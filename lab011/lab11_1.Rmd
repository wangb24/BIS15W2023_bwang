...
```{r message=FALSE, warning=FALSE}
homerange <- read_csv(here("lab011", "data", "Tamburelloetal_HomeRangeDatabase.csv"), na = c("", "NA", "\\"))
```
...
plt +
  geom_histogram(alpha = 0.4, color = "#323232", fill = "#34ccff") +
...
```{r}
plt +
  geom_density(color = "#323232", fill = "#34ccff", alpha = 0.4) +
  labs(title = "Distribution of Home Range Area")
```
...
homerange_range %>%
  ...
  geom_bar(position = "dodge", alpha = .35) +
  labs(
    title = "Observations by Taxon and Range Category in Homerange Data",
    x = "Range Category",
    fill = "Taxon"
  )
```
...
homerange_range %>%
  ...
  labs(title = "Small Home Range Animals",
    x = "Taxon",
    y = "Log10 Body Mass"
  )
```
